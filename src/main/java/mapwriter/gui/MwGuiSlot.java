...
public abstract class MwGuiSlot {
    ...
    /**
     * What to multiply the amount you moved your mouse by (used for slowing down scrolling
     * when over the items and not on the scroll bar)
     */
    ...
    public void actionPerformed(GuiButton button) {
        ...
        if (button.enabled) {
            final int slotHeight = this.getAverageSlotHeight();
    ...
    public void drawScreen(int mouseXIn, int mouseYIn, float f) {
        ...
        if (this.getEnabled()) {
            ...
            this.drawBackground();
            final int k = this.getScrollBarX();
            final int l = k + this.scrollBarWidth;
            this.bindAmountScrolled();
            ...
            final int j1 = this.top - (int) this.amountScrolled;
            ...
            final int k1 = this.func_148135_f();
    ...
    public int getAverageSlotHeight() {
        ...
        for (int index = 0; index < this.getSize(); ++index) {
            final int slotheight = this.getSlotHeight(index);
            height += slotheight;
    ...
    public int getSlotIndexFromScreenCoords(int x, int y) {
        ...
        final int k = this.left + this.width / 2 - this.getListWidth() / 2;
        final int l = this.left + this.width / 2 + this.getListWidth() / 2 - this.scrollBarWidth;
        final int i1 = y - this.top - this.headerPadding + (int) this.amountScrolled;
            ...
            if (Mouse.isButtonDown(0) && this.getEnabled()) {
                if (this.initialClickY == -1.0F) {
                    ...
                    if (this.mouseY >= this.top && this.mouseY <= this.bottom) {
                        final int i = this.width / 2 - this.getListWidth() / 2 + this.left;
                        final int j = this.width / 2 + this.getListWidth() / 2 + this.left - this.scrollBarWidth;
                        final int k = this.mouseY - this.top - this.headerPadding + (int) this.amountScrolled;
                        final int l = this.getSlotIndexFromScreenCoords(this.mouseX, this.mouseY);

                        if (this.mouseX >= i && this.mouseX <= j && l >= 0 && k >= 0 && l < this.getSize()) {
                            final boolean flag1 = l == this.selectedElement && Minecraft.getSystemTime() - this.lastClicked < 250L;
                        ...
                        final int i2 = this.getScrollBarX();
                        final int i1 = i2 + this.scrollBarWidth;
    ...
    /**
     * Scrolls the slot by the given amount. A positive value scrolls down, and a negative
     * value scrolls up.
     */
    ...
    /**
     * Sets the left and right bounds of the slot. Param is the left bound, right is calculated
     * as left + width.
     */
    ...
    protected void drawSelectionBox(int x, int y, int mouseXIn, int mouseYIn) {
        ...
        for (int index = 0; index < i1; ++index) {
            ...
            if (this.showSelectionBox && this.isSelected(index)) {
                final int xLeft = this.left + this.width / 2 - this.getListWidth() / 2;
                final int xRight = this.left + this.width / 2 + this.getListWidth() / 2 - this.scrollBarWidth;
    ...
    /**
     * The element in the slot that was clicked, boolean for whether it was double clicked or
     * not
     */
